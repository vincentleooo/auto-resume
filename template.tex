...
\begin{document}
	...
	\begin{myitemize}[topsep=0ex, labelsep=0.1in]
		$for(experience)$
		\begin{paracol}{2}\myitem\itemheading{$experience.position$}\\\subheading{$experience.company$}\switchcolumn{\vspace*{5pt}\monthyear{SINGAPORE}\\\monthyear{$experience.year$}}\end{paracol}
		\begin{itemize}[itemsep=-0.4ex, topsep=-0.4ex]
	...
	\begin{myitemize}[topsep=0pt, labelsep=0.1in]
		$for(cca)$
		\begin{paracol}{2}\myitem\itemheading{$cca.position$}\\\subheading{$cca.name$}\switchcolumn{\item[]\monthyear{SINGAPORE}\\\monthyear{$cca.year$}}\end{paracol}
		\begin{itemize}[itemsep=-0.4ex, topsep=-0.4ex]
			$for(cca.description)$
	...
	\begin{paracol}{2}
		...
		\begin{skills}
			$for(skills)$
			\item {$if(small)$\small $endif$$skills$}
			$endfor$
		\end{skills}
		...
		\begin{skills}[itemsep=-0.4ex]
			$for(tools)$
			\item {$if(small)$\small $endif$$tools$}
			$endfor$
		\end{skills}
		$endif$
